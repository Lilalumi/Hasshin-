...
public class PaddlePowerUI : MonoBehaviour
{
    ...
    void Update()
    {
        if (paddlePower == null) return;

        if (paddlePower.IsOnCoolDown())
        {
            ...
            coolDownTimeRemaining = paddlePower.GetCoolDownTimeRemaining();
            powerCDTimer.text = FormatTime(coolDownTimeRemaining);
        }
        else
        {
            // Muestra Ready, oculta el retrato y el temporizador
